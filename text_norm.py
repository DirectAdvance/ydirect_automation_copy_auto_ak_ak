...
# ── ⛔ ЗАПРЕЩЁННЫЕ ФОРМУЛИРОВКИ КОНТЕНТА — ЕДИНЫЙ список на ВСЕ пути генерации ────────────────
# Правила Семёна: «в контенте мы не пишем про рассрочку» (2026-07) и «"Кредитное решение" писать
# нельзя» (2026-07-28). Раньше запрет жил двумя копиями (`text_gen._INSTALLMENT_RE` и
# `ai_agents._INSTALLMENT_RE`) — новая формулировка потребовала бы третьей. Теперь список ОДИН,
# а `text_gen`/`ai_agents`/`create_set_assets` только зовут хелперы ниже.
# ⚠️ Легальное НЕ трогаем: «кредит», «автокредит», «в кредит», «первый взнос 0 ₽»,
# «платёж от 9 000 ₽/мес», «одобрение за 30 минут» — под паттерны не подпадают.
BANNED_CONTENT_PATTERNS: tuple = (
    # рассрочка в любой форме (+ хвост «без переплат», иначе он остаётся сиротой)
    re.compile(r"(?i)\bрассрочк\w*\b(?:\s+без\s+переплат\w*)?"),
    # «кредитное решение» в любом падеже/числе («кредитного решения», «кредитные решения»)
    re.compile(r"(?i)\bкредитн\w*\s+решени\w*\b"),
)


def mentions_banned_content(s) -> bool:
    """True, если строка содержит запрещённую формулировку (рассрочка / кредитное решение)."""
    text = str(s or "")
    return any(p.search(text) for p in BANNED_CONTENT_PATTERNS)


def strip_banned_content(s) -> str:
    """Убрать запрещённые формулировки из строки, оставив остальной текст читаемым."""
    text = str(s or "")
    if not text:
        return ""
    for p in BANNED_CONTENT_PATTERNS:
        text = p.sub(" ", text)
...
def _bad_ad_title(s: str) -> bool:
    ...
    if re.search(r"(?i)\bтрейд-?ин\b[^.]{0,24}\b(?:1[0-9]{2}|[2-9][0-9]{2})\s*%", s):
        return True
    # Завышенный процент «до N% цены/стоимости авто» (N ≥ 100) — всегда стоп, в т.ч. через точку
    # («{brand} трейд-ин. До 150% цены авто» не ловилось старым паттерном из-за [^.]).
    if re.search(r"(?i)\bдо\s*(?:1[0-9]{2}|[2-9][0-9]{2})\s*%\s*(?:цены|стоимости)\s+авт", s):
        return True
...
def _bad_ad_text(s: str) -> bool:
    ...
    return (A.has_forbidden_claim(s) or _bad_credit_payment_range(s)
            ...
            # Завышенный процент «до N% цены/стоимости авто» (N ≥ 100) — трейд-ин 150% цены авто недопустим
            or bool(re.search(r"(?i)\bдо\s*(?:1[0-9]{2}|[2-9][0-9]{2})\s*%\s*(?:цены|стоимости)\s+авт", s))
