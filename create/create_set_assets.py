...
# Общие авто-слова в «якоре» — признак того, что якорь НЕ марка, а кусок общей фразы
# («новое авто», «машина в кредит»). Реальные марки/модели таких слов не содержат.
_GENERIC_ANCHOR_RE = re.compile(r"(?i)(?<![a-zа-яё])(авто|автомобил\w*|машин\w*)(?![a-zа-яё])")
...
def _trim_ad_line(s: str, maxlen: int) -> str:
    """Нормализация ОДНОЙ строки объявления перед выдачей в ads.add.

    Здесь же два инварианта контента, потому что это ЕДИНСТВЕННАЯ точка, через которую проходят
    все строки финальной сборки адаптива (`_dedup_cap`, `_upgrade_credit_titles`,
    `_finalize_text_line`) — фильтры уровня `text_gen`/`ai_agents` сюда уже не достают:
    1. ⛔ РАССРОЧКА запрещена в контенте (правило Семёна). Живой факт 2026-07-28: заголовок
       «Tenet в кредит в Краснодаре. Первый взнос 0 ₽. Рассрочка» (porg-pl6iavd5, 713096702) —
       слово приклеивал добивочный хвост `_fill_title` УЖЕ ПОСЛЕ `_strip_installment_text`
       в `_rsya_titles`. «Кредит»/«первый взнос»/«платёж от N ₽/мес» — легальны, их не трогаем.
    2. Первая буква — ЗАГЛАВНАЯ (правило Семёна). Живой факт: «новое авто в кредит. Первый
       взнос 0 ₽» — шаблоны `f"{brand} …"` подставляют anchor из середины фразы («новое авто»).
       `_cap_first` меняет ТОЛЬКО первый символ: «KIA», «LADA Vesta», «КАСКО», «трейд-ин»,
       «₽/мес» в середине строки не затрагиваются."""
    s = _strip_installment_text(str(s or "").strip())
    if len(s) > maxlen:
...
def _upgrade_credit_titles(items: list[str], cap: int = _RA_TITLES_CAP,
    ...
    # `_credit_title_anchor` вырезает «якорь» из ПЕРВОГО заголовка, поэтому в brand может приехать
    # не марка, а общая фраза («Купить новое авто в кредит…» → brand='новое авто'). Тогда
    # brand-шаблоны дают «новое авто в кредит…» и «Новый новое авто…» (живой факт porg-pl6iavd5).
    # Общее авто-слово внутри якоря = НЕ марка → идём в non-brand ветку с нормальными вариантами.
    brand_real = (bool(brand) and not brand_low.startswith("авто")
                  and not _GENERIC_ANCHOR_RE.search(brand_low))
    if not brand_real:
        # Тип сайта звучит в контенте (правило Семёна 2026-07-28): на «С пробегом» безмарочный
        # набор обязан говорить о Б/У, иначе заголовки универсальные («Авто в кредит от 9 000
        # ₽/мес») и по ним не понять, что продаются машины с пробегом. Ветвление — тем же
        # `_is_bu_site(site_type)`, что уже используется в brand-ветке ниже (не новый механизм).
        # ⚠️ На НОВЫХ авто эти строки не появляются вовсе — «с пробегом» там прямая ложь.
        # Все строки ≤56 симв. и без слова «новое» (исключающие маркеры не встречаются).
        variants = ([
            "Авто с пробегом в кредит. Первый взнос 0 ₽",
            "Купить б/у авто. КАСКО на 1 год бесплатно",
            "Платеж от 9 000 ₽/мес. Авто с пробегом в наличии",
            "Одобрение за 30 минут онлайн. Б/у авто",
            "Кредит от 15 банков онлайн. Подбор авто с пробегом",
            "Выгода до 45% на авто с пробегом. Узнайте условия",
            "Трейд-ин выше рынка. Оценка авто с пробегом онлайн",
            "Госпрограмма 2026. Кредит на авто с пробегом",
        ] if _is_bu_site(site_type) else [
        ...
        variants = [
            ...
            # 1: AUTO-CONTEXT, brand интегрирован. На Б/У маркер типа сайта («с пробегом»)
            # прямо в заголовке (правило Семёна 2026-07-28); длина = len(brand)+34 ≤ 56.
            (f"{brand} с пробегом. Кредит от 9 000 ₽/мес" if _is_bu_site(site_type)
             else f"{brand} в наличии. Кредит от 9 000 ₽/мес"),
...
def _upgrade_credit_texts(items: list[str], cap: int = _RA_TEXTS_CAP,
    ...
    if not brand or brand_low.startswith("авто") or brand_is_utp:
        # Б/У-сайт: тип сайта обязан звучать в тексте (правило Семёна 2026-07-28). Все строки ≤81.
        variants = ([
            "Авто с пробегом в кредит. Первый взнос 0 ₽. КАСКО на 1 год. Заявка онлайн.",
            "Б/у авто с платежом от 9 000 ₽/мес. Одобрение за 30 минут от 15 банков.",
            "Выгода до 45% на авто с пробегом. Трейд-ин выше рынка. Узнайте условия.",
        ] if _is_bu_site(site_type) else [
        ...
        variants = [
            ...
            # Б/У: маркер типа сайта в брендовом тексте. Строка короче исходной на 6 симв. запаса,
            # длинную марку добьёт `_finalize_text_line` (обрезка по слову), лимит 81 не рвётся.
            (f"{brand} с пробегом в кредит. Трейд-ин выше рынка. Выберите авто."
             if _is_bu_site(site_type)
             else f"{brand} в кредит от 15 банков. Трейд-ин выше рынка. Выберите авто."),
        ]
