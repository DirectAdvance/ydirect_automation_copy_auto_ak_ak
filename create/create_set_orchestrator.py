...
def create_set_response(deps: dict):
    ...
    try:
        ...
        # Статус чтения нужен ниже, чтобы отличить «в кабинете нет условий» (штатно) от
        # «карту не прочитали» (потеря) — см. блок видимости потерь.
        from .create_set_corrections import account_retargeting_probe as _ret_probe
        ret_map, _ret_status = _ret_probe(_st_token, login) if _st_token else ({}, "no_token")
        ...
        # Пустая карта условий означает, что НИ ОДНА структурная аудитория tp1/tp2/tp4/tp5 не
        # уедет; раньше это оставалось warning'ом позиции (режется до 5 на позицию) → джоба
        # оставалась зелёной при полной потере аудиторий.
        #
        # ⚠️ Пустая карта — ДВА разных случая, и валить их в один ⚠️ нельзя (Семён 2026-07-28):
        #   • `ok` — кабинет прочитан, условий ретаргетинга в нём нет. Это ШТАТНО: правило
        #     Семёна «если нет в кабинете, то не добавляем в кампании». Гейт НЕ поднимаем,
        #     пишем информационную строку в errors_log (она джобу зелёной быть не мешает).
        #   • `no_token` / `error:…` — карту прочитать НЕ удалось, аудитории теряются вслепую.
        #     Это настоящая потеря: гейт has_issues поднимаем, как раньше.
        if not ret_map:
            try:
                ...
                _aud_groups = 0
            if _aud_groups and _ret_status == "ok":
                _add_job_err(_job, (
                    f"аудитории структуры не применены: в кабинете {login} нет условий "
                    f"ретаргетинга — {_aud_groups} групп(ы) созданы без аудиторий (штатно: "
                    f"чего нет в кабинете, то не добавляем)"))
            elif _aud_groups:
                _note_loss("audiences_not_sent", (
                    f"аудитории структуры НЕ отправлены: карта условий ретаргетинга кабинета "
                    f"{login} не прочитана ({_ret_status}) — {_aud_groups} групп(ы) со "
                    f"структурными аудиториями останутся без них"),
                    count=_aud_groups)
