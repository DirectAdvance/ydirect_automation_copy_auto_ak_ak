...
- DoD 1.3 (ключи): в порядке — v5 даёт 8511 ключей в 84 группах и 4353 в 34;
- 20 warn — все `BUILD_LIVE_UNDERCOUNT`, ЛОЖНЫЕ (см. ниже);
- DoD 1.5a (фильтр каталога tp7) **в этом прогоне не проверяется**: созданы только ct0000 и
  ct0006 (не марочные, пустой фильтр там штатен), а брендовая `ТК - Haval` — тот самый failed=1.
