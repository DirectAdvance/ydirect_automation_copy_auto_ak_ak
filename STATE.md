...
**Сделано:**
- `KEYWORDS_REPAIR_WIPES_LIVE_KEYWORDS` закрыт ЖИВЫМ прогоном: кампания `713155623`
  (`porg-rgwzgo57`) `119 групп / 0 ключей / 119 пустых` → **`119 / 15877 / 0 пустых`**
  (сверка по v5, не по ответу добивки). Регрессионный тест `test_keywords_repair_no_wipe.py`
  (6), проверено что ЛОВИТ регресс: вернул `keywords=[]` → 1 failed, вернул фикс → 6 passed.
- `PACK_GKLESS_POSITION_READS_LEGACY_ONLY` (новый, коммит `96faf6d0`): позиция без `gk` читала
  только легаси `{slepok}.txt` и не видела per-group файлы своего ct. Фолбэк на объединение,
  только когда легаси пуст. До→после: ct0051 `0→604`, ct0885 `0→267`, ct0283 `0→57`.
  5 тестов, проверено что ловят регресс (2 failed без фолбэка).
- **Разблокированы 3 аккаунта**, которые до этого отбивались 409 на content-gap:
  `uy3huxcn` 67 кампаний, `nqavjicg` 105, `dmwfp3dk` 106 — **0 failed**.
- Сплошная сверка по 8 аккаунтам: **групп без кодера — 0**, расхождений «кодер ↔ автотаргетинг» —
  **0** (проверено 1175+442+1175+1175+1463+1570+1036+1036 групп).
- Разобраны issues живой верификации: `NO_IMAGES_LIVE` — докрутился delayed-циклом (сейчас 0);
  `NO_ADPRICE_LIVE` (30) — НЕ дефект, фиды помеченных кампаний пусты по ценам (проверено сверкой
  со всеми фидами аккаунта на той же куке); `BUILD_LIVE_UNDERCOUNT` — ложный.
- Тесты LXC101: **46 failed / 836 passed** (было 46/825), новых падений нет.

**⚠️ Две МОИ цифры по ходу оказались неверными (обе — детектор по ИМЕНИ группы, не по составу):**
1. «Товары KIA / Каталог Renault — товарные, ключи не нужны» — опровергнуто: в кампании
   `713157521` все 119 групп имеют ОДИНАКОВЫЙ состав (`TextAd+ShoppingAd+ListingAd`),
   ключи есть у 46 и нет у 73.
2. «TP5_NO_KEYWORD_DONOR_FALLBACK — 1 группа из 1349» — занижено: реально
   196 (`dmwfp3dk`) / 202 (`nqavjicg`) / 8 (`uy3huxcn`) / 3 (`rgwzgo57`).

**Осталось — ЖДЁТ РЕШЕНИЯ СЕМЁНА (код НЕ трогал, обе правки меняют создание на всех аккаунтах):**
1. `TP5_NO_KEYWORD_DONOR_FALLBACK` — донор `tp5 → tp2 → tp1`. Закрыл бы **60 из 148** пустых
   tp5-групп на `dmwfp3dk` (Lada 175, Volkswagen 250, Skoda 128, Nissan 177 фраз в tp2).
   Остальные **88 — фраз нет нигде**, это дефицит пака (зона харвестера, не код).
2. `X3_KS_VARIANT_KEEPS_AUTOTARGET_ONLY_GROUP` — х3 отдаёт один список групп всем 3 вариантам,
   поэтому группа «Автотаргет» попадает в чистый «КС» с `aoff` и без ключей = мёртвая.
   Предложение: отбрасывать такие группы в «КС»-варианте.

**Не сделано намеренно:** push (указание «делай всё кроме пуша»).
`tools/.tone_watcher_cursor` не коммичу — рантайм-курсор чужого фонового процесса.

## Сессия 2026-07-29 — РЕСТРУКТУРИЗАЦИЯ фазы 1-3 + QA + полный прогон

**Сделано:** корень `direct/` 274 → 107 файлов, `.py` 197 → 45. Создано 8 пакетов:
`create` 45, `copy_service` 35, `web` 20, `content` 12, `clients` 11, `core` 10, `repair` 9,
`slepki_code` 5. Все 12 точек входа systemd ОСТАЛИСЬ в корне. Тесты на LXC101 (py3.11):
**47 failed / 824 passed** против baseline `8270f37e` = 49/822 — регрессий нет.
13 юнитов active, 5 вкладок 302, 0 ошибок импорта в журналах.

**Полный прогон** `22f4f8ddd6b2` (scherbakova/Мультибренд, 27 позиций, `single_feed=true`,
`n=true`): **created=26 / failed=1**. Сверка с DoD по живому кабинету:
- 1292 группы, **групп без кодера — 0**;
- DoD 1.2a (кодер `aon/aoff` ↔ `relevanceMatch.isActive`): **0 расхождений из 1170 групп**
  (tp5 исключён — там `isActive=True` всегда, это норма);
- DoD 1.3 (ключи): ❌ НАРУШЕН — я объявил «в порядке» по ВЫБОРКЕ из двух tp2/tp4-кампаний
  (8511 и 4353), не проверив «Общие». Семён нашёл по кабинету: в tp1 «Общие» 8 групп,
  ключи только в 4 (см. TP1_OBSHIE_KEYWORDS_LOST);
- 20 warn — все `BUILD_LIVE_UNDERCOUNT`, ЛОЖНЫЕ (см. ниже);
- DoD 1.c (структура → создание 1:1): ❌ НАРУШЕН — в структуре 3 tp7, создано 2
  (см. TP7_STRUCTURE_NOT_1TO1). DoD 1.5a следом непроверяем: непустой фильтр каталога бывает
  только у брендовой позиции, а она и не создалась.

**Единственный отказ:** `SIZE_OF_VIDEO_CONTENTS_CANNOT_BE_MORE_THAN_MAX` на tp7 Haval —
предсуществующий дефект, НЕ от реструктуризации (диф в `uac_client.py` и
`create_set_master_product.py` — только строки импортов). Утром тот же набор проходил 9/9,
пока M3 спал и видео-пул (37 роликов) был не виден.

**ТРИ ЛОЖНЫЕ ТРЕВОГИ — не чинить, это свойства измерителей:**
1. `BUILD_LIVE_UNDERCOUNT` по измерению «группы» — `grid_content_verifier.py:480` СУММИРУЕТ два
   build-ключа `groups`+`adgroups`, ожидание удваивается (62 против реальных 31).
2. Grid `groups_for_edit.keyword_count` даёт **0 на кампаниях, где v5 показывает тысячи ключей**.
   Судить о ключах по edit-view НЕЛЬЗЯ — только v5 или `showConditions`.
3. `NO_KEYWORDS_LIVE` на группах «Товарная галерея» — у них ключей нет по дизайну.

**Осталось:** дефект видео-лимита tp7 (см. ERRORS_JOURNAL) · watchdog на больших наборах
(см. там же) · пакеты `create`/`content`/`web`/`clients`/`core` переехали, но внутренние
импорты остались длинными (`from ..X import`) — косметика, можно чистить отдельно.

> ⚠️ **ОТКРЫТЫЙ ВОПРОС ПО GIT (2026-07-29).** Правило зон записано в `CLAUDE.md` и действует
> ВПЕРЁД, но фактическая топология ему не соответствует и разошлась исторически:
> `ydirect_automation_copy_auto_ak_ak` опережает `main` на **221 коммит** и де-факто является
> стволом — на ней лежит создание РК, слепки, бот слепков, автоправила, контент, а не только copy.
> `main` репо `neurodirectologist` (зона «создание») стоит далеко позади.
> Проверено фактом: перенести работу на `main` черри-пиком **невозможно** — 8 из 8 коммитов
> конфликтуют (за 221 коммит переехали файлы и разъехались модули). Перемотку `main`
> Семён отклонил (2026-07-29). Остаётся разбор истории по зонам — отдельный проект.
> До его решения новые коммиты ложатся на текущую ветку; НЕ пушить без указания Семёна.
> Контент-зона в порядке: `tools/content_redactor_git.py` — scoped-экспортёр в репо
> `yandex_direct_content_redactor`, файлы там совпадают с рабочей копией.
