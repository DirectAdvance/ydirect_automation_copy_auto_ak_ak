...
# Единый переиспользуемый текст на все группы (решение Семёна 2026-07-10):
# — НЕ per-group (не брать texts[0] из AI — там может быть аварийный короткий fallback);
# — заполнен под лимит ≤81 символа;
# — кредитный угол обязателен (кредит/взнос/мес).
SHOPPING_DEFAULT_TEXT = "Авто в кредит от 9 000 ₽/мес. Первый взнос 0 ₽. Одобрение за 30 минут."
...
def _upgrade_credit_titles(items: list[str], cap: int = _RA_TITLES_CAP) -> list[str]:
    ...
    if not brand_real:
        ...
        variants = [
            f"{anchor}. Кредит и первый взнос 0 ₽",   # brand + город
            f"{brand}. КАСКО на 1 год бесплатно",       # brand без города
            f"{anchor}. Платеж от 9 000 ₽/мес",        # brand + город
            f"Купить {brand} в кредит. Одобрение за 30 мин",  # Купить + brand
            f"{brand} в кредит. Кредит от 15 банков",  # brand + кредит
            f"{anchor}. Выгода до 45% при покупке",    # brand + город
            f"{brand}. Трейд-ин до 150% цены авто",    # brand без города
            f"{brand}. Госпрограмма 2026 и кредит",    # brand без города
            f"Купить {brand}. Заявка за 1 минуту",     # Купить + brand
        ]
