...
def _upgrade_credit_titles(items: list[str], cap: int = _RA_TITLES_CAP) -> list[str]:
    ...
    if not brand_real:
        ...
        variants = [
            f"{anchor}. Кредит и первый взнос 0 ₽",                   # 0: brand+город+finance
            f"{brand}. Авто в наличии. Кредит от 9 000 ₽/мес",        # 1: AUTO-CONTEXT + credit
            f"{anchor}. Платеж от 9 000 ₽/мес",                       # 2: brand+город+finance
            f"Купить {brand} в кредит. Одобрение за 30 мин",          # 3: action+brand+credit
            f"{brand}. Новые автомобили. Выгода до 45%",               # 4: AUTO-CONTEXT + discount
            f"{anchor}. Выгода до 45% при покупке",                   # 5: brand+город+discount
            f"{brand}. Трейд-ин до 150% цены авто",                   # 6: brand+tradein (авто упомянут)
            f"{brand}. КАСКО на 1 год бесплатно",                      # 7: резерв (finance)
            f"{brand} в кредит. Кредит от 15 банков",                 # 8: резерв (finance)
            f"{brand}. Госпрограмма 2026 и кредит",                   # 9: резерв (state)
            f"Купить {brand}. Заявка за 1 минуту",                    # 10: резерв (action)
        ]
