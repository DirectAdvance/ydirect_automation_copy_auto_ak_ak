...
def _ag_part1_map() -> dict:
    ...
    2. public.leadgen_ct_naming — leadgen-слепок dmp. Структура dmp сейчас использует
       ...
       ВЫДЕЛЕННЫЕ ct08xx вне авто-пространства (Конкуренты=ct0834). Naming gap ЗАКРЫТ 2026-07-18:
       ct0800–ct0833 (темы tp2) и ct0834 «Конкуренты» (МК tp6) имеют имена в leadgen_ct_naming,
       имена сверены со структурой dmp 1:1. Ни одно из них НЕ проходит _coder_name_real_brand,
       поэтому на выбор контента не влияют (остаётся общий B2B) — они для подписи/расшифровки ct.
    Кэш на процесс."""
