...
def _load_account(
    ...
    *,
    ...
    for s in sets:
        ...
        # ad-level наборы: перепривязка ResponsiveAd через v5/Grid не поддерживается
        # (см. _v5_rebind_ads_sitelink_set) — считаем, СКОЛЬКО и КАКИХ конкретно
        # объявлений это реально затронет, чтобы фронт показывал точное предупреждение,
        # а не безусловное «на всякий случай» для любого ad-level набора.
        responsive_count = 0
        responsive_examples: list[dict] = []
        if not camp_ids:
            seen_pairs: set[tuple] = set()
            for entry in ads_by_set.get(sid, []):
                if entry.get("subtype") in ("TextAd", "DynamicTextAd"):
                    continue
                responsive_count += 1
                pair = (entry.get("campaign_name") or "", entry.get("adgroup_name") or "")
                if pair not in seen_pairs and len(responsive_examples) < 5:
                    seen_pairs.add(pair)
                    responsive_examples.append({"campaign_name": pair[0], "adgroup_name": pair[1]})
...
def _replace_sitelink_text_grid(
    ...
    *,
    ...
    for target in targets or []:
        ...
        campaign_ids = [
            cid for cid in campaign_ids
            if cid not in touched_campaigns
            and (not _have_current_map or current_set_by_campaign.get(cid) == source_set_id)
        ]
...
def _do_replace(token: str, login: str, typ: str, old_text: str, new_text: str,
    ...
    if typ == "sitelink_reorder":
        ...
        if isinstance(payload, list):
            perm, target_set_id = payload, None
        else:
