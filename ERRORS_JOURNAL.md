...
- Статус: 🔴 НЕ исправлено, причина не найдена (нужен раскоп пути пак -> `only_cts` -> AddKeywords).
...
- Статус: 🔴 НЕ исправлено. Блокирует брендовые tp7, а с ними и проверку DoD 1.5a.
- НЕ помогло ранее: — (первая фиксация сигнатуры).
