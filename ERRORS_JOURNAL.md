...
### TP1_X3_SIBLING_LOST_KEYWORDS — одна из трёх х3-сестёр создалась без ключей, докрутка её не берёт (2026-07-30) 🟠
- Симптом: `porg-nqavjicg`, две х3-сестры одной позиции, ОДИНАКОВЫЕ группы:
  - `713157153 tp1 — РСЯ - Комби - Б/У - КС` → **699 ключей**, пустых групп 2 (норма);
  - `713157156 tp1 — РСЯ - Комби - Б/У - КС + Автотаргетинг` → **0 ключей, 8 групп из 8 пустых**.
  В паке фразы для этих групп ЕСТЬ: Autoru 66, Avitobu 178, Oficdealer 72, Avtoprobeg 168,
  Avtosalon 141, Drom 108. Это НЕ случай `---autotargeting` (там per-group файл пуст) —
  здесь файл несёт реальные фразы, а группа в кабинете пуста.
- Джоба `ee956279a325` по ОБЕИМ кампаниям записала `ok=true`, без errors — расхождение
  возникло уже ПОСЛЕ отчёта позиции, штатной диагностикой не видно.
- ⛔ Почему автодокрутка не помогла: `keywords_repair` ограничен `_SEARCH_TPS = {2, 4, 5}`
  (`repair/repair_common.py:24`) — **tp1 в его область не входит**, все 8 групп ушли в
  `skipped` (`status=200 ok=True repaired=None skipped=8`). Формально это дизайн (докрутка
  задумана под поисковые tp), но по факту tp1-кампании с «КС» в имени ключи НЕСУТ, и чинить
  их нечем: ни автодокрутка, ни delayed-цикл сюда не дотягиваются.
- Ремонт 2026-07-30 (разовый, тем же механизмом Grid `AddKeywords`, что и docкрутка): долито
  733 фразы в 6 групп; 2 группы («Фид», «Notype») пропущены — фраз в паке нет.
  Проверка по v5 ПОСЛЕ: **`713157156` = 699 ключей, пустых 2** — побайтово тот же профиль,
  что у здоровой сестры `713157153`. (733 → 699: Директ дедуплицирует/нормализует.)
- Не выяснено: ПОЧЕМУ ключи не доехали при создании именно в вариант «КС + Автотаргетинг».
  Кандидаты: частичный сбой Grid `AddKeywords` для второй кампании х3 без записи в errors.
  Разовый ремонт симптом снял, корень открыт.
- Предложение (НЕ применял, меняет область докрутки на всех аккаунтах): добавить `1` в
  `_SEARCH_TPS`, чтобы tp1-кампании с «КС» в имени попадали под `keywords_repair`.
- Статус: 🟠 симптом устранён на живом аккаунте, корень открыт, предложение по докрутке ждёт решения.

### NO_ADPRICE_LIVE на terehov-аккаунтах — НЕ дефект, фид пуст по ценам (проверено 2026-07-30) ✅
- Симптом: живая верификация `porg-nqavjicg` (18 warn) и `porg-dmwfp3dk` (12 warn) —
  `NO_ADPRICE_LIVE` на tp1-кампаниях «… - Модели», «Мультибренд - Модели», «Б/У - Модели».
  Живая сверка по всем tp1: без adPrice **18 из 83** и **12 из 83** кампаний с адаптивными объявлениями.
- Проверка (а не предположение): `_grid_feed_offer_prices` по ВСЕМ фидам обоих аккаунтов.
  У `nqavjicg` фид `3595507 yandex-used-auto` — тот самый, к которому привязаны помеченные
  кампании, — отдаёт **0 ключей с ценой**; у `dmwfp3dk` то же у `direct_feed.xml` и
  `dostup-k-rasprodazhe-live-01-b.xml`. При этом соседние фиды тех же аккаунтов по ТОЙ ЖЕ куке
  читаются нормально (2–6 ключей), то есть это свойство фида, а не сбой чтения.
- Марки помеченных групп (Lada Granta, Hyundai Creta, KIA Cerato, Renault Duster,
  Volkswagen Polo) в их фиде отсутствуют.
- Вердикт: ровно поведение из `DOD.md:766` — «марки нет в фиде → цена ПУСТАЯ `(0,0)`,
  НЕ фолбэк на минимум». severity=warn выставлена верно, чинить нечего.
- ⚠️ Для будущих проверок: 0 ключей от `_grid_feed_offer_prices` САМ ПО СЕБЕ ничего не доказывает —
  функция возвращает `{}` и при исключении. Вывод делать только сверив с другими фидами того же
  аккаунта на той же куке.

### KS_CAMPAIGN_HOLDS_AUTOTARGET_ONLY_GROUPS — группы, помеченные `---autotargeting`, стоят в кампаниях с «КС» в имени (2026-07-30) 🟠 структурный перенос
> Прежний заголовок записи — `TP5_NO_KEYWORD_DONOR_FALLBACK` (гипотеза «у tp5 нет донора»). Гипотеза ОПРОВЕРГНУТА, см. ниже.
- Симптом: `porg-rgwzgo57`, `tp5_cpc_site — Поиск + Динамика + Товарная галерея - Модели - КС +
  Автотаргетинг`: 82 группы, 13558 ключей, и ровно ОДНА пустая —
  `ct0099_aon_n000_r0121_ct010_ag011_g00 — Geely Atlas Pro`.
- Факт по паку (terehov/«С пробегом», ct0099): `tp5` — файлов слепка **0**, фраз 0;
  при этом `tp2` — **194** фразы, `tp1` — **306**. Контент есть, просто не в tp5.
- Где: `kontent_pack.py:1364-1368` — донорский фолбэк включается только для `tp2`/`tp4`
  (`tp4 → tp2 → tp1`, `tp2 → tp1`). `tp5` в условии отсутствует, хотя это тоже ПОИСКОВЫЙ тип
  с «КС» в имени. Та же цепочка продублирована в `slepki_code/slepki_editor.py:634`.
- Смягчающее: у группы `aon` — автотаргет включён, то есть она не мертва (в отличие от
  `X3_KS_VARIANT_KEEPS_AUTOTARGET_ONLY_GROUP` ниже). Но по DoD поисковая группа должна нести ключи.
- Намеренность исключения tp5 НЕ подтверждена: ни комментария, ни теста, фиксирующего её, нет.
  Комментарий рядом объясняет смысл фолбэка ровно как «не создавать кампании без КС, когда ключи
  лежат только в РСЯ» — под tp5 это подходит так же, как под tp4.
- ⛔ ВАРИАНТ «расширить донора до tp5» ОТКЛОНЁН 2026-07-30 — он НАВРЕДИЛ БЫ. Донор залил бы
  в группу ct-уровневые фразы ВОПРЕКИ явной пометке источника (см. опровержение ниже).
- ⚠️ МАСШТАБ ПЕРЕСМОТРЕН 2026-07-30 (первая оценка «1 группа из 1349» была занижена — она
  считалась детектором, который отбрасывал группы по ИМЕНИ, а не по составу объявлений):
  сплошной пересчёт с делением пустых групп на 4 корзины (норма в автотаргет-кампаниях /
  товарные / без объявлений / текстовые в КС-кампании) дал **196 на `porg-dmwfp3dk`,
  202 на `porg-nqavjicg`, 8 на `porg-uy3huxcn`, 3 на `porg-rgwzgo57`**.
- Ключевое доказательство, что это НЕ «товарные группы по замыслу»: в кампании `713157521`
  (`dmwfp3dk`, tp5, 119 групп) состав объявлений у ВСЕХ групп ОДИНАКОВ —
  `GdAdaptiveTextAd + GdShoppingAd + GdListingAd`. Ключи есть у 46 групп и отсутствуют у 73.
  То есть пустые — тот же тип групп, что и заполненные; отличие только в наличии фраз в паке
  под tp5. Имена «Товары X» / «Каталог X» вводят в заблуждение: по составу это не товарка-only.
- 🔴 **ОПРОВЕРГНУТО 2026-07-30 — дефекта КОДА тут нет, вывод «донор помог бы 60 группам» неверен.**
  Промежуточная проверка «tp5=0, а tp2 даёт 175/215/237/250 фраз» НИЧЕГО не доказывала: под tp2
  per-group файла для этих групп нет вовсе, поэтому чтение падало на ЛЕГАСИ ct-уровневый
  `terehov.txt` и возвращало общие фразы всего ct — а не фразы ЭТОЙ группы. Я принял артефакт
  легаси-фолбэка за «контент есть, просто не в том tp».
- Что на самом деле: у этих групп В tp1/tp5 ЕСТЬ свой per-group файл, и в нём ровно
  `---autotargeting` (16 байт, без единой фразы). Пример:
  `С пробегом/tp1/ct0164/keywords/terehov__каталог_kia.txt` = `---autotargeting`,
  `terehov__товары_kia.txt` = `---autotargeting`, при этом соседний `terehov__kia.txt`
  несёт `---autotargeting` + реальные фразы («kia цены», …). То есть источник ЯВНО помечает
  «Каталог KIA» / «Товары KIA» как автотаргет-только.
- Сплошная проверка (`porg-dmwfp3dk`, все пустые группы кампаний с «КС» в имени):
  **196 из 196 — `---autotargeting` в паке**. Категорий «per-group файл с фразами, а в кабинете
  пусто» — **0**. Создание отдаёт 0 ключей ПРАВИЛЬНО, по явному указанию источника.
- Почему на вкладке «Структура слепка» кажется, что ключи есть: на уровне **ct** карточка
  показывает легаси-агрегат (`tp1/ct0164` без группы = **626** фраз), а у конкретной ГРУППЫ — 0.
  Проверено тем же вызовом, что рисует UI (`slepki_editor.read_group_keywords`): UI и создание
  дают ОДИНАКОВЫЙ результат 0 для группы и 626 для ct. Расхождения UI↔создание НЕТ.
- Остаётся РЕАЛЬНАЯ проблема, но она СТРУКТУРНАЯ, а не кодовая: автотаргет-только группы стоят
  в кампаниях, у которых в ИМЕНИ «КС». Лечится переносом таких групп в структуре слепка в
  кампанию только с автотаргетингом (решение Семёна 2026-07-30), а не правкой чтения пака.
- Статус: ✅ по коду закрыто (дефекта нет); 🟠 открыт структурный перенос групп.

### X3_KS_VARIANT_KEEPS_AUTOTARGET_ONLY_GROUP — х3 переносит группу «Автотаргет» в чистый «КС»-вариант, где она не таргетирует НИЧЕГО (2026-07-30) 🟠 ОТКРЫТО, нужно решение Семёна
- Симптом: `porg-rgwzgo57` (terehov), кампания `tp1_cpc_site — РСЯ - Комби - Общие запросы - КС`:
  1 группа `ct0000_aoff_n000_r0121_ct001_ag011_g00 — Автотаргет`, **0 ключей**. Автотаргет
  выключен (`aoff` — верно для «КС»-варианта), ключей нет → показов не будет ни по чему.
  Соседи по х3 здоровы: `- Автотаргетинг` и `- КС + Автотаргетинг` идут с `aon` (0 ключей там норма).
- Где: `create/create_set_plan.py:1095-1099` — при теге «х3» `_emit_tp1` получает ОДИН И ТОТ ЖЕ
  список групп `_og` для всех трёх вариантов `X3_VARIANTS` (`create_set_structure.py:31-35`).
- Механика: у группы «Автотаргет» (`gk=автотаргет`) источник ключей — только `---autotargeting`
  (в паке `terehov__*` реальных фраз для неё нет). В вариантах с `autotarget=True` это корректно:
  группа живёт на автотаргете. В варианте «КС» (`autotarget=False`) у неё не остаётся ни ключей,
  ни автотаргета. Структура слепка тут ни при чём: `camp_names` группы «Автотаргет» содержат
  только «… - Автотаргетинг» и «… - КС + Автотаргетинг», чистого «… - КС» среди них НЕТ —
  «КС»-вариант порождает сам х3.
- ⚠️ Это НЕ то же, что `KEYWORDS_REPAIR_WIPES_LIVE_KEYWORDS`: там ключи были и их стёрли, здесь
  ключей не было никогда. Добивка такую группу чинить не должна — фраз для неё не существует.
- Варианты решения (нужен выбор, семантика создания меняется для ВСЕХ аккаунтов):
  (а) в «КС»-варианте х3 отбрасывать группы без источника ключей; если групп не осталось —
      не создавать и саму кампанию (риск: расхождение с «структура→создание 1:1» по DoD);
  (б) оставлять таким группам `aon` даже в «КС»-варианте (риск: имя кампании «КС» перестаёт
      соответствовать фактическому таргетингу — ровно та рассинхронизация имени и кодера,
      из-за которой уже был ложный `WRONG_AUTOTARGET`);
  (в) ничего не менять, считать пустую группу допустимым побочным эффектом х3.
- Рекомендация: (а) — не создавать заведомо мёртвые объекты; но это правка класса
  «review-first» (влияет на состав создаваемых кампаний), поэтому молча не делаю.
- Статус: 🟠 задокументировано, код НЕ трогал. Масштаб на `porg-rgwzgo57`: 1 такая группа
  из 1349 проверенных.
...
- Статус: ✅ ПОДТВЕРЖДЕНО ЖИВЫМ ПРОГОНОМ 2026-07-30 на той самой кампании `713155623`
  (`porg-rgwzgo57`, terehov). До: `групп=119 ключей=0 пустых=119`. Прогон
  `execute_keywords_repair` (штатные deps): `status=200 ok=True repaired_adgroups=119
  failed=0 unfixable=0`. После (независимая сверка по Direct API v5, не по ответу добивки):
  **`групп=119 ключей=15877 пустых_групп=0`**. Четыре группы усечены 319/269/325/253 → 200
  (штатный лимит Яндекса на группу), это ожидаемо и залогировано.
- Регрессионный тест: `tests/test_pack_gkless_keywords_fallback.py` — нет, ключевой здесь
  `tests/test_keywords_repair_no_wipe.py` (6 тестов, коммит `944b3397`): round-trip живых фраз,
  запрет литерала `keywords=[]` в исходнике, пропуск `keywords_truncated`-кампании целиком,
  «КС без Автотаргетинг не флагается». Проверено, что тест ЛОВИТ регресс: временно вернул
  `keywords=[]` → 1 failed; вернул фикс → 6 passed. Полный набор 46 failed / 831 passed
  (было 46/825), новых падений нет. Отчёт: `.claude/sdd/keywords-repair-wipe-fix.md`.
  ⚠️ Прежний статус этой строки был «🟡 ждёт живого прогона … offline-тест на fake Grid» —
  теста в дифе тогда не было вовсе (находка ревью), формулировка вводила в заблуждение.
...
- Симптом (нашёл Семён по кабинету): в кампании `tp1_cpc_site — РСЯ - Комби - Общие - КС`
  ...
  хотя в структуре слепка под неё 138+ фраз.
- Факт (v5 `keywords.get`, кампания `713151914`, исключён псевдоключ `---autotargeting`):
  `ct0001 Buynew` — 0, `ct0008 Avtoru` — 0, `ct0009 Avito` — 0, `ct0006 Avtocredit` — 0;
  `ct0007 Tradein` — 180, `ct0013 Oficdealernew` — 51, `ct0013 Avtosalon` — 103, `ct0010 Drom` — 4.
  Итого 338 ключей в 4 из 8 групп.
- В ИСТОЧНИКЕ ключи есть — `kontent_pack.gather("scherbakova","Мультибренд","tp1")`:
  ct0001 = 2 top-level / 43039 в подгруппах, ct0008 = 142/139, ct0009 = 165/176, ct0006 = 196/193,
  ct0010 = 176/156. То есть теряются между паком и Директом. `ct0010 Drom` теряет почти всё (176 -> 4).
- НЕ регресс реструктуризации: кампания `713140728` ДО переезда давала те же «338 ключей в 4 группах»,
  что и `713151914` после — цифры совпадают побайтово.
- ПРИЧИНА (найдена 2026-07-29): `_drop_foreign_city_keywords` (`city_morph.py:204`) и
  `_drop_used_car` (`core/automation_runtime.py:2935`) матчили ВСЮ строку ключа ВМЕСТЕ С
  МИНУС-ХВОСТОМ. Фраза `avito ru купить авто -уфа -липецк` падала из-за города в МИНУСАХ,
  который её как раз и запрещает в чужом городе; `buynew … -бу -пробег -подержанный` — из-за
  собственного запрета б/у. Фильтр читал ЗАПРЕТ как ПРИЗНАК.
  Потери: ct0001 −182, ct0006 −192, ct0008 −138, ct0009 −175, ct0010 −151.
  ⚠️ Молчало потому, что гейты нуля ключей КАМПАННЫЕ, а не групповые: в группу уезжал список
  из одного псевдоключа `---autotargeting` (truthy!), `rep["keywords"]`=338>0, `errors` пуст.
- Решение (2026-07-29): оба фильтра переведены на позитивную часть фразы (слова без ведущего `-`).
- Статус: ✅ ИСПРАВЛЕНО. Детектор `/tmp/detect.py`: групп с нулём после фильтра 4 → 0 (из 235).
  Обратная проверка: б/у-лексика в ПОЗИТИВНОЙ части просочившихся — 0 из 105788 ключей.
  Тесты 47 → 46 failed / 825 passed.
  ✅ ЖИВОЙ ПРОГОН `494ca4538dfc` (porg-pl6iavd5, 2026-07-30): кампания «Общие» — **1103 ключа
  в 8 группах из 8** (было 338 в 4). Сплошная проверка всех 27 кампаний: пустых поисковых
  групп (не галерея, не чистый автотаргет) — **0**.
- НЕ помогло ранее: — (первая фиксация сигнатуры).

### TP7_STRUCTURE_NOT_1TO1 — из 3 tp7 структуры создаётся 2 (2026-07-29) 🔴
- Симптом (нашёл Семён): в структуре слепка `scherbakova/Мультибренд` три tp7-кампании —
  `ТК - Автокредит - Аудитории + Автотаргетинг` (ct0006), `ТК - Общее - Автотаргетинг` (ct0000),
  `ТК - Haval - КС + Автотаргетинг` (ct0111). В кабинете после полного прогона `22f4f8ddd6b2`
  только две: `713151438` (ct0000) и `713151455` (ct0006). Брендовой Haval нет.
- Причина отсутствия третьей — `UAC_VIDEO_CONTENTS_OVER_MAX` (см. запись выше): единственный
  `failed=1` прогона.
- Нарушен DoD 1.c «Структура слепков → Создание РК 1:1». ⚠️ Побочно это же блокирует проверку
  DoD 1.5a (фильтр каталога tp7): непустой `listings_feed_filters` бывает только у БРЕНДОВОЙ
  позиции, а она и не создаётся — у ct0000/ct0006 пустой фильтр штатен.
- Статус: ✅ ИСПРАВЛЕНО вместе с UAC_VIDEO_CONTENTS_OVER_MAX. Прогон `494ca4538dfc`
  (2026-07-30): tp7 в кабинете **3 из 3** структуры. Следом ЗАКРЫТ и DoD 1.5a: у брендовой
  `ТК - Haval` `listings_feed_filters` = `collectionId EQUALS values:["mark_6"]`,
  `feed_filters` = `mark_id CONTAINS ["Haval","haval"]` — оператор EQUALS и непустые values,
  как требует канон.
- ⚠️ МЕТОДИЧЕСКАЯ ОШИБКА ПРОВЕРЯЮЩЕГО (моя): в отчёте по DoD я написал «1.5a не проверяется»
  вместо «нарушен 1.c», а DoD 1.3 объявил выполненным по ВЫБОРКЕ из двух tp2/tp4-кампаний
  (8511 и 4353 ключа), не проверив «Общие». Сверять DoD выборкой нельзя — только сплошняком
  по всем созданным кампаниям.

### UAC_VIDEO_CONTENTS_OVER_MAX — tp7 падает `SIZE_OF_VIDEO_CONTENTS_CANNOT_BE_MORE_THAN_MAX` (2026-07-29) 🔴
- Симптом: товарные tp7 не создаются, Яндекс отбивает `HTTP 400 ContentDefectIds.Gen.
  SIZE_OF_VIDEO_CONTENTS_CANNOT_BE_MORE_THAN_MAX` по полю `contentIds`.
  Прогоны: `cef19fbfaeb7` (9 из 9 tp7 Haval по фидам), `22f4f8ddd6b2` (единственный failed=1).
- НЕ от реструктуризации: диф в `clients/uac_client.py` и `create/create_set_master_product.py`
  за `8270f37e..HEAD` — только строки импортов, логика капа не тронута.
- Почему проявилось только сейчас: утром тот же набор проходил 9/9, потому что M3-Mac спал и
  видео-пул не был виден через sshfs. После пробуждения M3 в пуле стало **37 роликов**.
- Что проверено и НЕ является причиной: кап `[:2]` есть в обоих местах —
  `create/create_set_prefetch.py:525` (`for vp in (videos or [])[:2]`) и
  `clients/uac_client.py:708` (`spec.video_files[:2]`); сборщик картинок фильтрует строго по
  `_IMAGE_EXTS` (`clients/uac_client.py:153`), mp4 туда не попадает.
- Подозрение (НЕ подтверждено): ветка предзагрузки — при непустом `spec.content_ids`
  (`clients/uac_client.py:678-681`) весь блок загрузки, включая капы, ПРОПУСКАЕТСЯ, и список
  уходит в payload как есть. Нужен раскоп, чем именно наполняется `preloaded_content_ids`.
- ПРИЧИНА (найдена 2026-07-29): коммит `8b9f74aa` выдедентил ОБА видео-цикла из-под
  `if not content_ids` (`clients/uac_client.py`). На пути СОЗДАНИЯ prefetch уже кладёт в
  `preloaded_content_ids` 4 картинки + 2 видео → повторная заливка тех же mp4 давала 2+2=4
  видео при лимите Яндекса **2** (`DOD.md:439`). Накопления между джобами НЕТ (проверено:
  запись `it["preloaded_content_ids"] = …` присваивающая, кэш `uac_content_ids` write-only).
- ⚠️ Слепой откат ЛОМАЕТ копирование — тесты противоречат намеренно. Разведено по источнику:
  `video_files` (локальные) внутрь `if` — создание не льёт повторно; `video_urls` снаружи —
  копирование предзаполняет id картинок источника, а видео переносит ссылками.
- Статус: ✅ ИСПРАВЛЕНО. Оба контракта закреплены тестами и зелёные:
  `test_uac_preloaded_content_ids_skip_file_uploads` +
  `test_uac_client_uploads_video_urls_when_image_content_ids_are_preseeded`.
  ✅ ЖИВОЙ ПРОГОН `494ca4538dfc` (2026-07-30): **created=27 / failed=0** (было 26/1).
  Брендовая `ТК - Haval` `713153166` создана, у неё **4 картинки + 2 видео** (ровно лимит —
  видео не потерялись вместе с фиксом).
- НЕ помогло ранее: — (первая фиксация сигнатуры).

### WATCHDOG_FIRST_CAMPAIGN_FALSE_POSITIVE_ON_BIG_SET — сторож валит большой набор и НЕ останавливает воркер (2026-07-29) 🟡
- Симптом: набор из 82 позиций (`cf763c49f33f`, `single_feed=false` → товарка ×9 фидов) закрыт
  как `error` с `result.error = "watchdog: ни одной кампании за 15 мин и никаких признаков
  работы последние 14 мин"`, `first_campaign_timeout=true`, при `created=0 failed=0 done=0/82`.
- Причина: порог меряет время до ПЕРВОЙ кампании, а фаза подготовки контента растёт вместе с
  набором. На 82 позициях дедуп и заливка картинок (600+ файлов) не укладываются в 15 минут.
- ⚠️ Хуже симптома: после вердикта сторожа **воркер продолжает работать**. В журнале шли
  `[img-cache] MISS->upload total=200` уже ПОСЛЕ того, как джоба стала `error`. Осиротевший
  процесс пришлось гасить рестартом юнита. На боевом аккаунте это значит, что кампании могут
  создаваться под уже закрытую джобу.
- Обход: канонический полный набор — `single_feed=true`, 27 позиций (`22f4f8ddd6b2`) — проходит.
- Статус: 🟡 НЕ исправлено, зафиксировано.
- НЕ помогло ранее: — (первая фиксация сигнатуры).
...
- ⚠️ ДИАГНОЗ 2026-07-28 БЫЛ НЕВЕРЕН — дефицита контента НЕТ, был дефект ЧТЕНИЯ пака (см. следующую
  запись `PACK_GKLESS_POSITION_READS_LEGACY_ONLY`). Проверка «прямой вызов read_keywords → positive=0»
  ничего не опровергала: она повторяла тот же баг, что и гейт (обе шли через `group=""`). Ключи всё
  это время лежали рядом в per-group файлах: ct0051 — 604, ct0885 — 267, ct0283 — 57.
  Отдельно: `ct0885` был назван «пустым двойником ct0185» — это тоже неверно, у него свой контент.
- Статус: ✅ ЗАКРЫТО 2026-07-30 фиксом чтения пака (коммит `96faf6d0`), а НЕ top-up'ом контента.
  Гейт по всем трём аккаунтам — чисто.

### PACK_GKLESS_POSITION_READS_LEGACY_ONLY — позиция без gk не видит per-group ключи своего ct (2026-07-30) ✅
- Симптом: `create_set_pack_gap_note` отбивал HTTP 409 «нет keywords для tp2/ct0051, tp4/ct0051…»
  на `porg-uy3huxcn`, `porg-nqavjicg`, `porg-dmwfp3dk`, хотя файл `salamahin__chery_tiggo.txt`
  (261 КБ, 604 фразы) лежит прямо в `<сегмент>/tp2/ct0051/keywords/`.
- Root-cause: `kontent_pack._read_keywords_exact` при `group=""` читала ТОЛЬКО легаси
  `{slepok}.txt`. Позиция структуры без `gk` семантически = «весь ct», но пак разложен по
  per-group файлам `{slepok}__{slug}.txt`, а легаси-файла у этих ct просто нет → «ключей нет».
  Пар без gk много (salamahin: 163 из 208), так что дефект молчал только потому, что у остальных
  ct легаси-файл случайно существовал.
- Решение: для `group=""` — фолбэк на ОБЪЕДИНЕНИЕ всех `{slepok}__*.txt` этого ct
  (`_read_all_group_keywords`, обход через `listdir_bounded` — слепой `os.listdir` вешает процесс
  под sshfs). Берётся ТОЛЬКО когда легаси пуст: непустой легаси остаётся точным ответом.
  `*_minus.txt` уходят в минус-слова, не в позитивы.
- Факт до→после (живое чтение): salamahin ct0051 `0→604`, ct0885 `0→267`, terehov ct0283 `0→57`;
  ct0185 `139` не изменился. Гейт: все три аккаунта ЧИСТО.
- Тесты: `tests/test_pack_gkless_keywords_fallback.py` (5). Проверено, что ловят регресс —
  с отключённым фолбэком 2 failed. Полный набор 46 failed / 836 passed (было 46/831).
