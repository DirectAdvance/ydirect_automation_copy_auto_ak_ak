...
- Статус: 🟡 фикс на Mac (Mutagen синкнул). Сервисы НЕ рестартовались — ждёт рестарта `direct-create.service`/`direct-create-worker.service` и живого прогона tp2/tp4 при `DIRECT_API_FIRST`.
...
- Статус: 🟡 фикс на Mac (Mutagen синкнет). Сервисы НЕ рестартовались (шёл боевой прогон) — ждёт рестарта `direct-create.service`/`direct-create-worker.service` и живого прогона.
