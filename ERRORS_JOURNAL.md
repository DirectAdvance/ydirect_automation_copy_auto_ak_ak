...
- Решение (2026-07-27): общий хелпер `model_urls._is_degenerate_feed_url` (пусто / голый корень / путь `/quiz`) — ОДИН на все 4 пути, ставится ПОСЛЕ фид-лукапа и ДО `link_check`. Вырожденный фид игнорируется, href строится формулой `_model_page_href`. В tp2/tp4 дополнительно проброшен `_uname` (зеркало tp1-обхода `_multi and _uname`): при пустом `real_brand` формула идёт по структурному имени группы. `strip_quiz_url` не трогали. Легальный `/auto` (UAZ нет в фидах → `/auto/uaz` → 404 → `_parent_path`) под гард не попадает: он рождается ПОСЛЕ гарда, в resolve-цепочке, а гард смотрит только на URL из фида.
- Верификация: `py_compile` 5 файлов OK; 219 passed по всем тестам, трогающим изменённые модули (8 новых: таблица гарда, квиз-оффер tp2, каталог-оффер без изменений, `uname`-фолбэк, tp1 без `_multi`, гард-контракт tp6/tp7, посевы, UAZ→`/auto`).
- Статус: 🟡 ждёт рестарта `direct-create.service`/`direct-create-worker.service` и живого прогона. Ожидание на `porg-pl6iavd5`: `ROOT_HREF 0`, `/auto` остаётся 2 (UAZ, легально). Детектор: `lxc101:/tmp/_detect_root_href.py <логин>`.
...
- Статус: 🟡 ждёт живого прогона с `failed>0`. Проверять: в `direct_automation_jobs.result` джобы со `status=error` есть `has_issues.lv_errors`/`ver_errors`, совпадающие с `live_verification.summary.errors` / `verification.summary.errors`.
- НЕ помогало ранее: — (первое наблюдение этой сигнатуры).
...
- Статус: 🟡 код В ПРОДЕ по основной части: файлы доехали 18:02 +05, `direct-create.service`/`direct-create-worker.service` рестартованы 2026-07-27 18:08:31/33 +05. Лог непроведённых коллбэков добавлен ПОСЛЕ рестарта — эта строка доедет со следующим рестартом. Ждёт подтверждения живым прогоном.
...
- Статус: 🟡 ждёт рестарта `direct-create.service`/`direct-create-worker.service` и живого прогона. Доработка ревью — тоже 🟡: локально `direct/tests` 494 passed, 3 pre-existing fail (copy_rename ×2, m3_lock — падают и без правки).
