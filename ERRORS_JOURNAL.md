...
### TP4_DONOR_FALLBACK_AUTO_LOST_WHEN_DONOR_IS_MARKER_ONLY — `_pack_group_fact` терял флаг автотаргетинга донора, когда донор нёс ТОЛЬКО маркер `---autotargeting` (2026-07-30) 🟡 ждёт прогона живого создания

- Симптом: `create/blueprint_targeting.py:_pack_group_fact` — донор-фолбэк `tp4→tp2→tp1`/`tp2→tp1`
  копировал `real`/`reals`/`auto` из первого донора с `d_real > 0`, но если у донора в цепочке
  единственная непустая строка — сам маркер `---autotargeting` (`d_real=0, d_auto=True`), условие
  `if d_real > 0:` его молча пропускало → `auto` не подхватывался, оставался `False`, хотя у донора
  автотаргетинг реально задан. Найдено ревью `direct_neyrodirektolog` сканом живого пака на LXC101:
  **3476 из 25542** (13.6%) tp1/tp2-файлов содержат ТОЛЬКО маркер, ни одной реальной фразы —
  массовый, не edge-case сценарий.
- Первая точечная правка (`if d_real > 0 or d_auto:` + `break`) сама внесла НОВЫЙ регресс, пойманный
  живым тестом: для `tp4` цепочка донора двухшаговая (`tp2→tp1`), и `break` на первом же marker-only
  доноре (`tp2`) обрывал поиск раньше, чем цепочка доходила до `tp1`, где реальные ключи ЕСТЬ. Живой
  пример `Мультибренд/salamahin/ct0036/changan_cs95`: tp2-донор marker-only, tp1-донор несёт 30
  реальных фраз — наивный break давал `real=0` вместо корректных `real=30` (было бы РЕГРЕССОМ
  относительно старого поведения, которое эти 30 фраз находило).
- Решение (итоговое): `d_auto` копится по ВСЕЙ цепочке доноров в `chain_auto` (без break); `break` и
  присвоение `real`/`reals` происходит только когда найден донор с `d_real > 0` (как раньше); если
  реальных ключей не нашлось ни у одного донора цепочки — `for...else` подхватывает `chain_auto` в
  `auto`, `real`/`reals` остаются `0`/`[]`.
- Проверено:
  1. `python3 -m py_compile create/blueprint_targeting.py` — OK.
  2. Синтетический кейс (marker-only донор, native real=0, БЕЗ каскада) — было `auto=False`
     (реконструкция старого условия `if d_real > 0:`), стало `auto=True`, `real=0` не изменился.
  3. Живой каскадный кейс LXC101 `Мультибренд/salamahin/ct0036/changan_cs95` (tp4, donor tp2
     marker-only → donor tp1 реальные 30 фраз): наивная правка `break` на первом marker-only доноре
     давала `real=0` (регресс); итоговый фикс с `chain_auto` вернул `real=30, auto=True` — совпадает
     со старым каскадным поведением по `real`, плюс корректный `auto`.
  4. Регресс «чистая КС» (донор без маркера, `d_real>0`, `d_auto=False`) — `auto` остаётся `False`
     как и раньше, не задет (синтетический тест).
  5. Регресс «донор пуст полностью» (нет ни маркера, ни ключей ни у одного донора цепочки) —
     `real=0, auto=False`, не задет.
- Живой прогон создания РК с этим кодом (проверка что группа реально получает `relevanceMatch.isActive`
  правильно в кабинете) — НЕ выполнен в этой сессии; изменение только на уровне чтения факта пака
  (`_pack_group_fact` питает UI-бейджи и планирование, не сам движок создания групп напрямую —
  нужно сверить с `slepki_editor.read_group_keywords`, откуда мигрировала эта логика).
- НЕ помогало ранее: буквальная правка `if d_real > 0 or d_auto: ... break` без накопления `chain_auto`
  по цепочке — теряет реальные ключи более дальнего донора в двухшаговой цепочке `tp4: tp2→tp1`.
- Статус: 🟡 код исправлен и покрыт синтетическими + одним живым read-only тестом чтения пака, ждёт
  подтверждения `direct_neyrodirektolog` и живого прогона создания.
...
### GIT_GUARD_PREFLIGHT_BLOCKS_INSTEAD_OF_AUTOHEALING_SAFE_BEHIND — preflight звал на помощь вместо самоисцеления в безопасном случае (2026-07-30) 🟡 4 сценария прогнаны на тестовом репо, живой прод-прогон LXC101 не требовался
- Симптом: за одну сессию 4 задачи Agent Board (#65/#67/#68/#69) блокировались одним и тем же
  классом причины — `direct_git_guard.py preflight` видел `behind>0` против `origin/<branch>` и
  ВСЕГДА возвращал `BLOCKED`/`return 3`, даже когда рабочее дерево было полностью чистым и не было
  локальных неотправленных коммитов (случай, который можно починить `git reset --hard` без потери
  данных).
- Root-cause: до сегодня это было оправдано (см. `GIT_TOOLS_NO_AUTH_CANNOT_FETCH_PUSH` выше —
  `git fetch`/`push` на LXC101 физически не могли аутентифицироваться, поэтому доверять origin как
  единственному источнику истины было нельзя). После фикса auth это ограничение снялось, но
  `cmd_preflight` не подтягивал новую логику.
- Решение: в `tools/direct_git_guard.py::cmd_preflight` при `behind>0` теперь проверяются ДВА
  условия безопасности — весь `git status --porcelain` (без scope-фильтра) пуст И `ahead==0`
  (нет локальных коммитов, которых нет на origin). Оба выполнены → `git reset --hard
  origin/<branch>`, `AUTO-HEALED` в stderr, `exit 0`, дальше выполняется штатный scope-based
  dirty-check без изменений. Если дерево грязное ИЛИ `ahead>0` (diverged) — остаётся `BLOCKED`,
  `return 3`, с явным диагнозом причины; авто-мёрж diverged НЕ делается намеренно (риск конфликтов).
- Проверено фактом на временном тестовом git-репо (`/private/tmp/.../scratchpad/git-auto-heal-test`,
  НЕ прод-репо): 4 сценария — (1) чистое дерево + behind → `AUTO-HEALED`/`exit 0`/HEAD==origin;
  (2) грязное дерево (uncommitted+untracked) + behind → `BLOCKED`/`exit 3`/файлы не тронуты;
  (3) свой коммит не на origin + origin тоже ушёл вперёд (diverged) → `BLOCKED`/`exit 3`/упоминание
  "diverged"/коммит на месте; (4) всё синхронно → `exit 0` без `AUTO-HEALED`. Все 4 совпали с
  ожиданием. `py_compile` OK. Дополнительно живой прогон на Mac и LXC101 реального
  `home/seoadvanced/direct`: `behind=0` на обоих (не дошло до auto-heal ветки, что и ожидалось —
  ветка уже синхронна), `BLOCKED` там был только от НЕсвязанных чужих uncommitted slepki-файлов
  (не трогал).
- **НЕ помогало ранее:** н/д (первый фикс этой сигнатуры — до сегодняшнего auth-фикса поведение
  "всегда BLOCKED на behind" было осознанно правильным, не багом).
- Отчёт: `.claude/sdd/git-auto-heal-report.md`.
...
- Причина: в конфиге прогона `variants` были `tp1_rsy, search_test, search_dynamic,
  search_gallery, product_auto, product_manual`. `product_*` — это **tp7 (Товарные)**;
  для **tp6 (МК)** нужен отдельный вариант `master_*`. Планировщик ставит `want_master`
  только при `str(v).startswith("master")` (`create/create_set_plan.py:1459`), поэтому tp6
  не попал в план вовсе — хотя `tp_sq` для «6» в конфиге был прописан и вводил в заблуждение.
- ⚠️ Система ПРЕДУПРЕЖДАЛА, я не посмотрел: `create_set_plan.py:1635-1640` кладёт в ответ
  плана warning «структура слепка содержит tp6 (МК, N позиций), но master-вариант не выбран
  в запросе — tp6 не будет создан». В логах прогона я грепал только
  `PLAN count|START http|FINAL|live_verification|JOB_ID` — предупреждения плана не читал.
- Урок на будущее: **warnings ответа `/direct/api/set_plan` читать ВСЕГДА**, они называют
  ровно то, что молча не будет создано. Грепать вместе с FINAL.
- tp6 в структурах (то есть сколько потеряно): terehov/«С пробегом» — 21, terehov/Мультибренд
  — 25, salamahin/Мультибренд — 8, kuderko/«С пробегом» — 2; у scherbakova tp6 нет (0) —
  на `4ealp4ry`, `azsw6eyh`, `pl6iavd5` создавать нечего.
- Исправление: отдельный прогон ТОЛЬКО с `variants=["master_auto","master_manual"]`,
  `tp_sq={"6":["site"]}` (без CPA), существующие типы не пересоздаются. План 79 кампаний
  (2 + 7 + 26 + 22 + 22), Терехов в конце.
