...
- НЕ трогали: ретрай `CAMPAIGN_NOT_FOUND` (eventual consistency после `campaigns.add`, идемпотентен по смыслу), реавторизацию по 403 — протухшая кука по-прежнему переподхватывается, разведение «read вернул `{}`» vs «read упал» (`*_strict`-читатели), исключение `AddKeywords`.
