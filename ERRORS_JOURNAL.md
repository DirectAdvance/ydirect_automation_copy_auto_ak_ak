...
- Статус: 🟡 код УЖЕ В ПРОДЕ: файлы доехали Mutagen'ом 18:02 +05, `direct-create.service`/`direct-create-worker.service` рестартованы 2026-07-27 18:08:31/33 +05, в живом процессе `inspect.signature(_create_text_via_token)` содержит `keep_keywords`. Ждёт подтверждения живым прогоном tp2/tp4 при `DIRECT_API_FIRST`.
...
- Статус: 🟡 гейт на месте, детект-запрос как метод проверки зафиксирован; живым прогоном по API-пути после рестарта ещё не подтверждён.
...
- Статус: 🟡 фикс на Mac (Mutagen синкнет). py_compile OK. Тесты: 38/38 passed (4 новых: Phase 1.5 exception→error, верификатор aon/aoff). Ждёт живого прогона. Доработка 2026-07-27 (вердикт ❌ проверяющего): (1) Phase 1.5 exception теперь проставляет rep["error"] (singular) → _cleanup_partial сносит черновик вместо ok=True с неверным isActive. (2) добавлен комментарий legacy-страховки _AUTOTARGET_KW. (3) grid_read.py: новый счётчик wrong_autotarget_rsya_groups для tp1 (aon/aoff vs isActive). (4) grid_content_verifier.py: tp1-ветка WRONG_AUTOTARGET по wrong_autotarget_rsya_groups.
...
- Статус: 🟡 фикс на Mac, py_compile OK (3 файла). Ждёт деплоя и живого прогона (scherbakova/Мультибренд single_feed=True).
