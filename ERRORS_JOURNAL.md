...
### TP5_NO_KEYWORD_DONOR_FALLBACK — у tp5 нет донорской цепочки, поисковая группа остаётся без ключей (2026-07-30) 🟠 ОТКРЫТО, нужно решение Семёна
...
- Вариант решения: расширить цепочку до `tp5 → tp2 → tp1` (и синхронно в редакторе слепков).
  Структуру и состав кампаний это НЕ меняет — только заполняет пустую группу. Риск: в поиск
  приедут фразы, собранные под РСЯ (тот же риск уже принят для tp2/tp4).
...
- Сколько именно починил бы донор (пересчёт по паку для КАЖДОЙ пустой tp5-группы `porg-dmwfp3dk`,
  8 tp5-кампаний, 148 пустых групп):
  - **60 групп — донор помог бы**: `tp5=0`, но `tp2` даёт фразы (ct0181 Lada — 175,
    ct0238 Volkswagen — 250, ct0215 Skoda — 128, ct0199 Nissan — 177);
  - **88 групп — фраз нет нигде** (`tp5=0`, `tp2=0`, `tp1=0`) — это реальный дефицит пака,
    кодом не лечится, зона `direct_content_harvester` / top-up контента.
  То есть донор закрыл бы ~40% пустых tp5-групп, остальное — контент.
- Статус: 🟠 задокументировано, код НЕ трогал — правка меняет источник ключей для tp5 на ВСЕХ
  аккаунтах.
