...
4. **Сайтлинки — ОДИН набор на ЛОГИН, переиспользовать во ВСЕХ кампаниях.** Сейчас различаются по
   ...
   Флаг `DIRECT_SITELINK_REUSE_ACCOUNT` → **включить** + проверить, что механизм реально шарит один набор.
...
| 2.1c | **Авто-контекст в заголовках** (Д1 2026-07-10, прогон af4bd7bd5a52) — из 7 заголовков **1–2 ОБЯЗАНЫ** явно сообщать, что продаём АВТОМОБИЛИ («авто в наличии», «новые автомобили», «{марка} в наличии»). Запрещено делать ВСЕ 7 только про кредит/финансы — пользователь должен понять продукт. Остальные 5 — кредитный угол (по DoD §2). | `ai_agents.build_titles_messages`: добавлено правило «1–2 из {TITLES_N} — авто-продукт». `create_set_assets._upgrade_credit_titles` brand_real=True: позиции 1 и 4 (из cap=7) заменены на `f"{brand}. Авто в наличии. Кредит от 9 000 ₽/мес"` и `f"{brand}. Новые автомобили. Выгода до 45%"` (Д1 2026-07-10). Live-аудита НЕТ (только генерация + детерминированный upgrade). | 🟡 (код есть, **live не проверено**; чинит только будущие прогоны) |
...
⚠️ **НЕ охвачено** (инфра-граница, не мои файлы): `create_set_feed_builders.py:_create_tp5_single`
(строки 709, 733) — hardcoded fallback `"Официальный дилер. Тест-драйв и выгодные условия по кредиту.
Авто в наличии."` (содержит «по кредиту»), и `_tp5_account_data` (строка 636) читает `texts[0]` из
`direct_slepok_content`. Эти строки требуют правки в `direct_neyrodirektolog` (infra-зона).
...
- [ ] Каждый текстовый/UAC tp = **ПАРА кампаний** `cpc` + `cpa`.
...
- [ ] Пара `cpc`(`PER_CLICK`) + `cpa`(`PER_CONVERSION`) — поле `pricing`.
...
- [ ] Заголовки плотные (≥48/56): код `SHORT_TITLES` авто-добивает суффиксами (`campaign_spec_audit.py:943`).
...
- [ ] Пара `cpc` + `cpa`; метрика `counters`+`goals`; UTM на кампании.
...
|---|---|---|---|---|
| **Автотаргет** | — | — | не применимо | глобальные `_enabled_minus_words` |
...
- ✅ **Минус-слова на tp7:** как на tp6 — в UAC-payload ВСЕГДА (глоб. `_enabled_minus_words` во всех
  режимах; минус-КС слепка — только ручной `keywords`). НЕ путать с фид-минус-марками (`it_ff`/`it_lff`,
