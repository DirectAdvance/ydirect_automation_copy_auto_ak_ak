...
| 1.3c | **Ручной режим tp6/tp7 `audience` реально несёт аудитории в кабинете.** Эталон — структура слепка (`tp67_struct_expectations` → `audiences`); в кабинете аудитории лежат в `ca_retargeting_condition.condition_rules[].goals` (НЕ в `ca_retargeting_condition.goals` и НЕ в плоском `audiences` — оно приходит `null`). ⚠️ Ловушка проверки: `summarize_uac_detail.audiences` даёт **tri-state** — `None` означает «поле не прочитано» и ГАСИТ проверку, а не «аудиторий нет». Реальный ноль обязан приходить как `0`. Именно из-за путаницы этих двух состояний детектор `UAC_STRUCT_AUDIENCES_MISSING` не срабатывал никогда, а живая кампания с 9 аудиториями выглядела пустой. | `UAC_STRUCT_AUDIENCES_MISSING` (error при `expected>0, live==0`) / `..._UNDERCOUNT` (warn при `0<live<expected`) | `UacReadClient.campaign_detail` → `summarize_uac_detail` → `audiences`; эталон — `create_set_context.tp67_struct_expectations` | `uac_read._count_audiences` обходит `condition_rules[]`; тесты `tests/test_uac_audience_counter.py` | ✅ (2026-07-30: счётчик починен, `porg-xjxpfxby` 713160868 — структура 9 / кабинет 9) |
