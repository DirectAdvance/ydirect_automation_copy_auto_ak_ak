...
from ..slepki_code import slepki_editor as _sed
...
def _job_agency(job: dict) -> str:
    ...
    agency = ((job.get("body") or {}).get("agency") or job.get("agency") or "").strip().lower()
    if agency:
        return agency
    login = (job.get("login") or (job.get("body") or {}).get("login") or "").strip()
    return (_resolve_agency_hint(login, "") or "").strip().lower() if login else ""


def _ensure_body_agency(login: str, body: dict | None) -> str:
    """Persist real agency into web-posted job body before DB claim/adopt.

    Most UI paths already resolve this before queueing. This fallback protects service
    callers/tests and prevents an empty agency key from bypassing the cross-process gate.
    """
    if not isinstance(body, dict):
        return ""
    agency = str(body.get("agency") or "").strip().lower()
    if not agency:
        agency = (_resolve_agency_hint((login or body.get("login") or "").strip(), "") or "").strip().lower()
        if agency:
            body["agency"] = agency
    return agency
...
def _job_new_web(total: int, login: str, body: dict, saved_session: dict,
    ...
    if body is not None:
        _ensure_body_agency(login, body)
...
def _copy_retry_body_from_failed(row: dict) -> dict:
    ...
    original_user = (
        body.get("_copy_retry_original_user")
    ...
    body["_copy_retry_original_user"] = str(original_user or "").strip()
...
def _copy_worker_result_from_copy_job(copy_job: dict) -> dict:
    """Return the result persisted by the shared queue worker for a copy job.

    copy_engine keeps the rich diagnostic payload in _COPY_JOBS[jid]["result"] for both
    done and error terminal states. The shared worker must not collapse error results to
    {"error": "..."} only, otherwise scoped retry/debug loses workdir/id_maps/verification data.
    """
    cj = dict(copy_job or {})
    data = cj.get("result") if isinstance(cj.get("result"), dict) else {}
    out = dict(data) if isinstance(data, dict) else {}
    status = str(cj.get("status") or "")
    err = str(cj.get("error") or "").strip()
    if err:
        out.setdefault("error", err)
    if status not in _JOB_TERMINAL:
        out.setdefault("error", err or "копирование не завершилось")
    if not out:
        out = {"error": err or "копирование не завершилось"}
    return out
        ...
        try:
            with app.app_context():
                ...
                for row in rows:
                    ...
                    retry_jid = _job_new(total, login, body, {}, dedup_login=False, priority=True)
        ...
        try:
            ...
            if _is_edit_job:
                ...
                try:
                    ...
                    data = _copy_worker_result_from_copy_job(cj)
                except Exception as e:  # noqa: BLE001
                    data = {"error": str(e)[:300]}
            else:
            ...
            if _job_final is not None:
                ...
                if final_status == "done" and not _is_delete_drafts and not _is_edit_job:
                    ...
                    # Задача F: в DIRECT_ASYNC_FINALIZE=ON захваченные финализации → очередь
                    # finalize_set. В DIRECT_BATCH_ASPECTS=only orchestrator уже применяет их
                    # синхронным batch UpdateCampaigns и снимает recorder.
                    ...
                    try:
                        ...
                        if (_rec is not None and _rec.specs
                                and _finalize_queue_module().async_finalize_enabled()):
