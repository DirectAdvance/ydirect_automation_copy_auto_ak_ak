...
try:
    from . import grid_campaign_edit as _campaign_edit
except ImportError:                     # плоский запуск (локальные тесты из direct/)
    import grid_campaign_edit as _campaign_edit  # type: ignore[no-redef]
...
# Лимит карусели в UpdateAdaptiveTextAds: Grid валидирует размер списка и отвергает ВЕСЬ item
# при выходе за границы (`CollectionDefectIds.Size.SIZE_MUST_BE_IN_INTERVAL`, params
# {minSize: 2, maxSize: 10}, живой отказ 2026-08-04 на porg-nhzrd5xc).
GRID_MULTICARDS_MIN = 2
GRID_MULTICARDS_MAX = 10


def grid_multicards_for_write(raw_multicards) -> list[dict] | None:
    """Карусель для RMW-payload или None, если ключ ``multicards`` слать НЕЛЬЗЯ.

    Одна карточка — не карусель: Grid такой размер не принимает и заваливает весь апдейт
    объявления (тексты/картинки/цена тоже не доезжают). Ретрай-лестница
    ``create_set_feeds._send`` это уже разгребала третьим заходом «without-multicards»,
    то есть тем же отказом от ключа — но ценой двух заведомо провальных вызовов Grid на
    объявление (за 7 дней: 1145 отказов ради 120 успешных откатов). Отсекаем на входе.

    Больше максимума — обрезаем до ``GRID_MULTICARDS_MAX``: часть карусели лучше, чем
    отвергнутый апдейт целиком.
    """
    cards = _grid_multicards_write(raw_multicards)
    if len(cards) < GRID_MULTICARDS_MIN:
        return None
    return cards[:GRID_MULTICARDS_MAX]
...
class GridClient:
    ...
    def __init__(self, login: str, cookie: str | None = None, cookie_only: bool = False,
                 refresh_explicit_cookie: bool = False):
        self.login = login
        ...
        self._refresh_explicit_cookie = bool(refresh_explicit_cookie)
    ...
    def _reauth(self) -> None:
        ...
        self.csrf = None
        try:
            if not self._explicit_cookie or self._refresh_explicit_cookie:
    ...
    def post_idempotent(self, op: str, query: str, variables: dict, *,
        ...
        Повторяем: транспортный сбой (исключение requests), HTTP 5xx, non-JSON ответ и ответ 200
        с транзиентной ошибкой Директа. Ошибки валидации маркеров не содержат → возвращаются
        вызывающему сразу, с прежним поведением. Исчерпали попытки — возвращаем ПОСЛЕДНИЙ ответ
        (вызывающий разберёт его и бросит свою ошибку, как раньше); если ответа не было ни разу —
        ...
        for attempt in range(attempts):
            ...
            try:
                ...
                body = re.sub(r"\s+", " ", str(getattr(resp, "text", "") or ""))[:160]
                print(
                    f"[grid] {op}: не-JSON HTTP {resp.status_code} "
                    f"ctype={ctype!r} body={body!r}, попытка {attempt + 1}/{attempts}",
                    flush=True,
                )
                continue
    ...
    def _read_unified_campaign_update_payloads(self, campaign_ids: list[int]) -> dict[int, dict]:
        ...
        for chunk in [ids[i:i + 50] for i in range(0, len(ids), 50)]:
            rows = self._campaigns_edit_data_rows_resilient(chunk)
            for row in rows:
                try:
    ...
    def _campaigns_edit_data_rows_once(self, chunk: list[int]) -> tuple[list[dict], list]:
        return _campaign_edit.campaign_edit_data_rows_once(
            post=self._post,
            login=self.login,
            query=_CAMPAIGNS_EDIT_DATA_Q,
            chunk=chunk,
        )

    def _campaigns_edit_data_rows_resilient(self, chunk: list[int]) -> list[dict]:
        """Read CampaignsEditData with retry and per-campaign fallback for transient Grid 500."""
        return _campaign_edit.campaign_edit_data_rows_resilient(
            post=self._post,
            login=self.login,
            query=_CAMPAIGNS_EDIT_DATA_Q,
            chunk=chunk,
            is_transient=_is_transient_data_error,
            error_cls=GridFinalizeError,
        )
    ...
    def set_campaign_callouts(self, campaign_ids: list[int], callout_ids: list[int | str]) -> list:
        ...
        """
        co_ids = _campaign_edit.unique_positive_ints(callout_ids)
        self._bootstrap_csrf()
        return _campaign_edit.narrow_update_campaigns(
            login=self.login,
            campaign_ids=campaign_ids,
            read_payloads=self._read_unified_campaign_update_payloads,
            narrow_bases=self._narrow_bases,
            post_update=self._post_json_retry,
            error_cls=GridFinalizeError,
            op="Grid set-callouts",
            log_prefix="set-callouts",
            apply_base=lambda cid, base: (
                base.__setitem__("inheritableCallouts", {"calloutIds": [str(i) for i in co_ids]})),
        )
    ...
    def set_campaign_sitelink_set(self, campaign_ids: list[int], sitelink_set_id: int | str | None) -> list:
        ...
        return _campaign_edit.narrow_update_campaigns(
            ...
            post_update=self._post_json_retry,
            error_cls=GridFinalizeError,
            op="Grid set-sitelink-set",
            log_prefix="set-sitelink-set",
            apply_base=lambda cid, base: (
                base.__setitem__("inheritableSitelinkSet", {"sitelinkSetId": sid_raw})),
        )
    ...
    def set_campaign_disabled_places(self, campaign_ids: list[int], hosts: list[str]) -> list:
        ...
        return _campaign_edit.narrow_update_campaigns(
            ...
            read_payloads=self._read_unified_campaign_update_payloads,
            narrow_bases=self._narrow_bases,
            post_update=lambda op, query, vars_: self._post(op, query, vars_).json(),
            error_cls=GridFinalizeError,
            op="Grid set-disabled-places",
            log_prefix="set-disabled-places",
        ...
        for chunk in [ids[i:i + 50] for i in range(0, len(ids), 50)]:
            ...
            for row in rows:
                ...
                out[cid] = {
                    ...
                    "minus_keywords": ([str(x) for x in (row.get("minusKeywords") or [])]
                                       if "minusKeywords" in row else None),
                    "bid_modifiers_read": "bidModifiers" in row,
                    "bid_modifiers_present": bool(row.get("bidModifiers")),
                    ...
                    "avg_cpa": _num(strat.get("avgCpa") or strat.get("filterAvgCpa")),
    ...
    def update_ad_images(self, ad_items: list[dict], *, allow_empty_images: bool = False) -> int:
        ...
        for it in (ad_items or []):
            ...
            if it.get("multicards"):
                # Размер вне [2..10] Grid отвергает вместе со ВСЕМ item'ом — тогда ключ не шлём
                _cards = grid_multicards_for_write(it.get("multicards"))
