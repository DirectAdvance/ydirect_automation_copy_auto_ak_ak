...
### Измерения UAC (tp6/tp7) — `copy_uac._copy_uac_verify_rows`

D1–D19 строятся из v5-снэпшота, которого у чистого UAC нет: раньше такой прогон давал ноль
измерений и карточка честно писала «сверка не выполнялась». 9 строк на кампанию, кладутся в
`result["uac_verify"]` ОТДЕЛЬНО от `copy_verify` — осевшая пере-сверка перезаписывает
`copy_verify` целиком через `run_copy_verification`, которая про UAC не знает.

| Измерение | Сверяет | Заметка |
|---|---|---|
| `uac_titles` / `uac_texts` / `uac_keywords` | spec ↔ live | обе стороны через `_copy_uac_strings` (дедуп + limit), иначе дубль в spec = ложный mismatch |
| `uac_images` | число хэшей источник ↔ цель | единственное сравнение с ИСТОЧНИКОМ |
| `uac_counter` / `uac_goal` | spec ↔ live | `counters` список, `goals` список словарей |
| `uac_regions` | spec ↔ live | поле `regions` |
| `uac_feed` | spec ↔ live | `null` у МК — это `ok`, а не `unreadable` |
| `uac_minus_keywords` | ОБЪЁМ, не пословно | Директ нормализует и схлопывает формы, порог 70% |

Сверяется НАМЕРЕНИЕ (`spec`) против сохранённого (`live`), а не источник против цели:
счётчик, цель, гео и фид у копии намеренно другие — в этом и смысл переноса в новый кабинет.

### Карантин новых измерений — `copy_verify_issues._OBSERVATION_DIMENSIONS`

Новое измерение сначала попадает туда: считается и пишется в `has_issues.observation_only`,
но карточку не красит. Правило заведено ценой трёх живых прогонов 2026-08-05 — свежая сверка
UAC трижды показала расхождение, и все три раза виновата была она сама. Убирать из карантина
после подтверждения на реальном прогоне.
