# Нейродиректолог — STATE.md архив (сессии старше 3 дней)

> Перенесено из STATE.md 2026-07-13 при заведении правила ротации (STATE.md держим коротким — последние ~3 дня активных сессий).
> Историческая справка, НЕ читать первым в начале сессии — актуальное состояние в STATE.md.

## Сессия 2026-07-10 — ПЕРЕСБОРКА 13 СЛЕПКОВ из реальных данных ✅ ДЕПЛОЙ (`dcf212b`+`63229c7`)
- **Корень бага «все слепки дают одинаковую структуру»:** `slepki_structure.json` был суперсетом с
  байт-идентичными секциями `items` у РАЗНЫХ слепков (cross-коллизии) → любой слепок = одинаковая структура.
- **Пересобрано (cross-slepok коллизии 6→0):**
  - Структура: ре-харвест всех 11 авто-слепков по вкл/выкл/**удалённым** аккаунтам (Grid-**перебор всех 6 кук**,
    лимит 20/слепок/тип); генератор `scripts/build_slepok_structure.py` фильтрует суперсет по реальному корпусу.
    Работают 3 куки: victoryagency14 / victoryagency-direct1618440 / useful-call-agency.
  - Мастера tp6/tp7: Grid-перебор по всем (karavaev 3 акк из porg-6fako7a2/zp4rlf6u/jxv3b5dm; gordeeva 55 real tp7);
    ⚠️ v5 НЕ видит UAC — только Grid-кука. karavaev tp7 = GdTextAdGroup (ручные текст+картинка, feedId=[]).
    Мастера karavaev-формата (tp6) в клиентских аккаунтах ОТСУТСТВУЮТ — живут в отдельных master-аккаунтах вне Victory DB.
  - terehov: структура по его Google-листу (tp6/tp7 восстановлены); лист = «что хочет» > корпус.
  - Голос AGENT_ADS (ai_agents.py): 13/13 звучат по-своему; gen_ses/dmp — из реальных выгрузок
    (cars-start.ru xlsx / dmp-ai-450833-qmf4 скрины МК). dmp = НЕ авто (B2B-лидоген) → исключён из cross-картинок.
  - Гейт targeting_profile.json синхронизирован; ключи karavaev tp6/tp7 (+1368) в tp67_real_keywords.json;
    пак scherbakova +2427 файлов; kuderko tp2 (правило: tp2 ВСЕГДА автотаргет aon).
  - Зачистка: 81 пустая группа/сплит удалены, 6 пустых tp убраны из профиля → пустых кампаний в прогоне НЕ будет.
- **Preflight:** `scripts/slepki_preflight.py` (cross-коллизии + пустые группы + пустые tp в профиле) — exit 0.
- **blueprint.py распил** (`63229c7`): 8544→8000, вынесены blueprint_targeting/metrika/content_rules (ре-экспорт, DI).
- **Контент-тест (#51):** 26 кейсов боевым M3 → Google Sheet (лист Семёна). Тон различается 5/9 чётко.
  Правки промпта (в работе): dmp тянет авто (build_titles_messages ~2377), ct0000 галлюцинация марки,
  Квиз не использует б/у, свободные символы = ошибка (заполнять бюджет 56/30/81 по лимитам Яндекса).
- **Осталось:** прогон реальный (по команде Семёна); лист терехова #49 (r0134/гео-URL/возраст/tp7-feed); проверить лист-правки.
...
- **Task #34 (Grid слеп на саб-аккаунтах):** `campaign.py:pick_working_cookie` теперь пробует УПРАВЛЯЮЩЕЕ
  ...
  Корень: `fetch_cookie_glavpotok("porg-*")`=404 → перебор без приоритета → первая живая, но чужая
  агентская кука → Grid «No rights» → верификатор слеп. Изолир.тест: victorylotsofads1 первым для psm5h7q6.
- **DEFECT 2/4 (tp7 <8 сайтлинков):** корень — рассинхрон дедупов: сборка master_product дедупит по
  паре (title,desc), а `campaign._norm_sitelinks` — по desc отдельно → резал → <8 в UAC. Фикс:
...
# Ротация 2026-07-16: сессии 2026-07-12 и 2026-07-11
...
## Сессия 2026-07-16 — Per-слепок минус-площадки — SUPERSEDED
- **SUPERSEDED 2026-07-19:** актуальная модель снова глобальная: UI и движок читают
  `public.direct_global_minus_places`, параметр `slepok` для `_enabled_minus_places(slepok)` сохранён
  только для совместимости и игнорируется. Страница слепков показывает/сохраняет общий список через
  `/direct/api/slepki/minus-places`; POST admin-only. Per-slepok helper'ы из runtime удалены.
