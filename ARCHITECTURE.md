...
|---|-------------|--------:|------------------|----------------|----------------|------|
| 1 | `llm_providers.py` | 5243; 9792-10138 | `_m3_complete(_url/_parallel)`, `_m3_llm_probe`, `_openrouter_api_key/_probe`, `_or_complete_url`, `_llm_pair_for` + `_M3_LLM_*`, `_OPENROUTER_*` | `copy_geo_morph` (инъекция `m3_complete`); `ai_agents` косв. | только локальный `_OPENROUTER_KEY_CACHE` (переносится) | **LOW** |
