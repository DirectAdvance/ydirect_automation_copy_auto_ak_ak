...
> ⚠️ **КОНТРАКТ ПАМЯТИ (грабля, 2026-07-19): `slepki_store.assemble()` по умолчанию отдаёт ЖИВОЙ
> кэш-объект — мутировать его НЕЛЬЗЯ.** Кэш общий на процесс (ключ = mtime+size частей), любая
> правка «на месте» протекает во всех читателей и переживает отказ preflight.
> • нужна правка → `assemble(mutable=True)` (deepcopy, ~0.09 с на 19.8 МБ) или
>   `slepki_editor._load_struct()` — у него `mutable=True` ПО УМОЛЧАНИЮ, чтобы флаг нельзя было
>   забыть; read-only обходы явно передают `mutable=False` и копию не платят (~0.3 мс);
> • **главный кандидат на грабли — shim `automation_runtime._json("slepki_structure.json")`**: он
>   перехватывает чтение удалённого монолита и возвращает `assemble()`, т.е. ТОТ ЖЕ живой объект.
>   Выглядит как «свежий json с диска», но им не является — мутировать результат `_json` для этого
>   имени нельзя (в тестах монкипатчить шим на `assemble(mutable=True)`).
